...
function JobOfferForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
      ...
      // Convert skills to array for both add and update
  const updatedFormData = {
    ...
    try {
      ...
      ? await axios.put(url, updatedFormData, { headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` } })
      : await axios.post(url, updatedFormData, { headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` } });

      if (response.data.success) {
        toast.success(
          editingJobId ? "Job updated successfully!" : "Job offer submitted successfully!",
          { position: "top-center" }
        );
        fetchJobs();
        setShowModal(false);
    ...
    <div className="container-main-job">
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {editingJobId ? "Update Job Offer" : "Post a Job Offer"}
                </h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                <form onSubmit={handleSubmit}>
                  ...
                  <button
                    type="submit"
                    className="btn btn-primary addjob-btn"
                    disabled={loading}
                  >
                    {loading
                      ? editingJobId
      ...
      <div className="job-list mt-2">
        {jobs.map((job) => (
          <div
            key={job._id}
            className="card-2 shadow border rounded mb-3 job-card-1"
          >
            <div className="card-body text-start">
              ...
              <p>
                ...
                {job.jobDescription.length > 150 && (
                  <button
                    className="btn btn-link p-0 ps-2"
